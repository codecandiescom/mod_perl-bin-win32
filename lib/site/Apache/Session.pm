...
$Apache::Session::VERSION = '0.10_01';
...
if (!$@) {
	...
	my $locker = tie (%sessions, IPC::Shareable, 'sess', {'destroy' => 0, 'exclusive' => 0 , 'create' => 1});
...
sub new {
	...
	#garbage collector
	foreach $session (keys(%sessions)) {
		
		if ( $sessions{$session}->{'meta'}->{'expiration_time'} < time() ) {
	...
	if ($sessions{$this_session}) {
		...
		$self = &create_new($this_session, $package);
