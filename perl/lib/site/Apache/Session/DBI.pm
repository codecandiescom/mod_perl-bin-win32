...
my $sth_get     = $dbh->prepare( "SELECT id, a_session FROM sessions WHERE id = ?" ) || die $DBI::errstr;
my $sth_destroy = $dbh->prepare( "DELETE FROM sessions WHERE id = ?" ) || die $DBI::errstr;
my $sth_create  = $dbh->prepare( "INSERT INTO sessions (id, a_session) VALUES ( ?, ? )" ) || die $DBI::errstr;
my $sth_update  = $dbh->prepare( "UPDATE sessions SET a_session = ? WHERE id = ?" ) || die $DBI::errstr;
